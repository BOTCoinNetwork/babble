...
func TestCatchUp(t *testing.T) {
	...
	nodes := []*Node{
		newNode(peers.Peers[0], keys[0], genesisPeerSet, peers, 10000, 1000, 10, false, "inmem", 10*time.Millisecond, false, "", t),
		newNode(peers.Peers[1], keys[1], genesisPeerSet, peers, 10000, 1000, 10, false, "inmem", 10*time.Millisecond, false, "", t),
		newNode(peers.Peers[2], keys[2], genesisPeerSet, peers, 10000, 1000, 10, false, "inmem", 10*time.Millisecond, false, "", t),
		newNode(peers.Peers[3], keys[3], genesisPeerSet, peers, 10000, 1000, 10, true, "inmem", 10*time.Millisecond, false, "", t),
	}
	defer shutdownNodes(nodes)
...
func TestFastSync(t *testing.T) {
	...
	nodes := initNodes(keys, peers, genesisPeerSet, 100000, 400, 5, true, "inmem", 10*time.Millisecond, false, "", t)
	defer shutdownNodes(nodes)
