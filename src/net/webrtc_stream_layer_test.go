...
func TestWebRTCStreamLayerWithWampSignal(t *testing.T) {
	...
	wampSignal1, err := wamp.NewClient(url, realm, "alice", certFile, false, signalTimeout, common.NewTestEntry(t, common.TestLogLevel))
	if err != nil {
		t.Fatal(err)
	}

	wampSignal2, err := wamp.NewClient(url, realm, "bob", certFile, false, signalTimeout, common.NewTestEntry(t, common.TestLogLevel))
	if err != nil {
		t.Fatal(err)
	}
